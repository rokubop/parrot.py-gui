...
When I want to increase a python version for parrot, I need to ensure it works on both Windows, Mac and Linux. This takes a while so I don't usually do this. Parrot supports Python v3.13. audiomentations, used for augmenting audio, capped at v3.12 until v0.42.0 lifted it.
...
For example, `audioop` was removed from Python 3.13, which is why `audioop-lts` is in both requirements files.