...
so a profile is simply a folder shaped like ``data/``:
    ...
    data-profiles/<name>/
        profile.json     marks the folder as a profile
...
``PARROT_DATA_DIR`` and relaunches the GUI; ``lib/default_config.py`` derives
every data path from that env var at import time, which is why a restart is
required.
...
def duplicate(source_data_dir, name):
...
def import_into(dest_data_dir, source_data_dir):
    ...
    the one unrecoverable act here, and it silently replaced notes and
    config along with the sounds. Every real path lands
...
def rename(name, new_name):
    ...
    the one thing that holds its old path: the current pointer.
