...
profiles are skipped (with a note) otherwise.
"""
import os
import shutil
import wave
...
def create_test_profiles():
    ...
    def build(name, labels_seconds, with_model):
        if name in existing:
    ...
    build("test-empty", [], False)
    build("test-2-sounds", [("pop", 60), ("hiss", 60)], False)
    build("test-10-sounds",
          [(label, 35) for label in ("pop", "hiss", "ah", "oh", "ee", "cluck",
                                     "tut", "shush", "guh", "err")],
          False)
    if pair:
        build("test-model", [("pop", 60), ("hiss", 60)], True)
        build("test-model-4-sounds",
              [("pop", 75), ("hiss", 75), ("ah", 75), ("oh", 75)], True)
    else:
        notes.append("no model found on this machine, so test-model and "
                     "test-model-4-sounds were skipped; rerun once any "
