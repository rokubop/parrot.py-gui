...
- every send is fire-and-forget UDP: if the GUI isn't running, datagrams
...
its overlay is open. Use one observer at a time; this bridge unhooks cleanly
via the ``parrot_gui_bridge_disable`` action if needed.
...
try:
    ...
    def _make_wrapper(delegate, original):
        def pattern_match_with_bridge(frame):
            ...
            try:
                ...
                pass
            return active
        pattern_match_with_bridge._parrot_gui_bridge = True
    ...
    def _wrap():
        ...
        if getattr(original, "_parrot_gui_bridge", False):
        ...
        print(f"[parrot_gui_bridge] wrapped parrot integration v{__version__}")
        return True
    ...
    @mod.action_class
    class Actions:
        def parrot_gui_bridge_enable():
            """Start publishing parrot frames to the Parrot.py GUI"""
            _wrap()

        def parrot_gui_bridge_disable():
            """Stop publishing parrot frames and restore the integration"""
            _unwrap()

    # Keep trying until the integration is loaded, then double as a heartbeat.
